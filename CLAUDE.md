...
- Site public (blueprint `public`, déjà en place mais vide depuis P0) : consomme **uniquement** les JSON de `data/published/` générés en P5 (jamais la base de données directement — c'est tout l'intérêt du découplage).
- Carte satellite (fond IGN déjà en place), tracé avec liseré clair, tronçons run/nage visuellement distincts (réutiliser l'approche `tronconsCoordinates`/dasharray de `carte-simulation.ts`, à généraliser dans `shared/` puisque cette fois le site public en a aussi besoin), flèches directionnelles (couche `symbol` MapLibre, `symbol-placement: line`).
- Tronçons empruntés plusieurs fois : décalage latéral (`line-offset`) par passage, pastilles numérotées, message contextuel selon la position du curseur (déjà calculable : `course-{id}.geojson` contient `passages_multiples` avec bornes en mètres).
- Slider kilométrique (0 à la longueur totale, pas de 10 m) avec bande colorée par type de tronçon et repères POI, marqueur qui suit `turf.along`, option « suivre le curseur » + lecture automatique.
- POI cliquables (popup desktop / bottom sheet mobile), lecture de `pois-{id}.json`.
- Sélecteur de course (onglets) si plusieurs courses publiées, métadonnées Open Graph, favicon depuis le logo (`docs/brand/logo.jpg`, déjà présent).
- Mobile-first, mais pas de nouveau composant qui duplique la logique déjà écrite pour l'orga (`temps.ts`, la géométrie de tracé) — les réutiliser depuis `shared/` en les y déplaçant si besoin.
- Plus tard : premier déploiement réel sur PythonAnywhere (compte à créer/fournir côté utilisateur), intégration du logo/captures dans `docs/brand/` pour extraire une vraie palette (actuellement palette provisoire), et validation visuelle en navigateur de tout ce qui a été construit sans outil de navigateur disponible depuis P3 (P3, P4, P5).
